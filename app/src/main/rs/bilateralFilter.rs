...
static float f(float4 in, float4 neighbour){
    ...
    float Xin = (0.7976f * in.r + 0.1351f * in.g + 0.0313f * in.b)/(0.7976f+0.1351f+0.0313f);
    float Yin = (0.2880f * in.r + 0.7118f * in.g + 0.00008f * in.b)/(0.2880f+0.7118f+0.00008f);
    float Zin = (0.0f * in.r + 0.0f * in.g + 0.8252f * in.b)/(0.8252f);

    float Xnb = (0.7976f * neighbour.r + 0.1351f * neighbour.g + 0.0313f * neighbour.b)/(0.7976f+0.1351f+0.0313f);
    ...
    float dist = sqrt(pown(Lin -Lnb,2)+pown(ain - anb,2)+pown(bin - bnb,2))/3;
    ...
    for(int i = x-intensity ; i < x+intensity+1 ; i++){//for each collumn of the mask
        for(int j = y-intensity ; j < y+intensity+1; j++){//for each line of the mask
            if(i >= 0 && j >= 0 && i < width && j < height){//checking if we are in the limits of the image
                ...
                //int64_t time = rsUptimeMillis();
                colorcoef = f(pixelin,pixeldata);
                //time = rsUptimeMillis() - time;
                //rsDebug("TIME",time);
